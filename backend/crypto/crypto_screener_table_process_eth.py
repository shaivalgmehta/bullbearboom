...
def update_screener_table_eth(selected_date=None):
    with get_db_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute("TRUNCATE TABLE crypto_screener_table_eth")

            # Step 2: Insert latest daily data (today or yesterday)
            cur.execute("""
                INSERT INTO crypto_screener_table_eth (
            ...
            cur.execute("""
                WITH latest_weekly AS (
                    ...
                    WHERE datetime > %s
                    AND stock IN (SELECT stock FROM crypto_screener_table_eth)
                    ORDER BY stock, datetime DESC
                )
                UPDATE crypto_screener_table_eth s
                SET 
...
if __name__ == "__main__":
    update_screener_table_eth()
    print("crypto_screener_table_eth has been updated successfully.")