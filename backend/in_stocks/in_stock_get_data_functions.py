...
def fetch_stock_list_twelve_data():
    """
    Fetch stock list from both BSE and NSE exchanges using separate API calls.
    ...
    for exchange in exchanges:
        try:
            url = f"https://api.twelvedata.com/stocks?country=India&type=Common Stock&exchange={exchange}&apikey={TWELVE_DATA_API_KEY}"
...
def fetch_stock_statistics_twelve_data(symbol):
    statistics = td.get_statistics(symbol=symbol, country="India").as_json()
    return statistics

def fetch_stock_cashflow_twelve_data(symbol):
    td = TDClient(apikey=TWELVE_DATA_API_KEY)
    cashflow = td.get_cash_flow(symbol=symbol, country="India", period="quarterly").as_json()
    return cashflow
...
def store_statistics_data(data):
    ...
    execute_values(cur, """
        INSERT INTO in_quarterly_table (
            datetime, 
            stock, 
            sales, 
            ebitda, 
