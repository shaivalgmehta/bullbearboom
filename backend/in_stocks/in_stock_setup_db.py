...
def setup_database():
    with psycopg2.connect(**DB_CONFIG) as conn:
        with conn.cursor() as cur:
            # Create IN screener_table
            create_table(cur, 'in_screener_table', """
                ...
                erp5_rank INTEGER,                
                CONSTRAINT in_screener_pkey PRIMARY KEY (datetime, stock)
            """)

            # Create insider trading table
            create_table(cur, 'in_insider_trading_table', """
                datetime TIMESTAMPTZ NOT NULL,
                stock TEXT NOT NULL,
                stock_name TEXT,
                insider_name TEXT,
                ...
                CONSTRAINT in_insider_trading_pkey PRIMARY KEY (datetime, stock, shares_traded, transaction_type, transaction_id, price_per_share, shares_owned_following)
            """)

            create_table(cur, 'in_alerts_table', """
                datetime TIMESTAMPTZ NOT NULL,
                stock TEXT NOT NULL,
                stock_name TEXT,
                oversold_alert TEXT,
                anchored_obv_alert_state TEXT,
                CONSTRAINT in_alerts_pkey PRIMARY KEY (datetime, stock)
            """)

            # Create IN daily time series table
            create_table(cur, 'in_daily_table', """
                datetime TIMESTAMPTZ NOT NULL,
                stock TEXT NOT NULL,
                stock_name TEXT,
                open NUMERIC,
                ...
                erp5_rank INTEGER,                 
                last_modified_date TIMESTAMPTZ NOT NULL,
                CONSTRAINT in_daily_pkey PRIMARY KEY (datetime, stock)
            """)

            # Create IN weekly time series table
            create_table(cur, 'in_weekly_table', """
                datetime TIMESTAMPTZ NOT NULL,
                stock TEXT NOT NULL,
                williams_r NUMERIC,
                williams_r_ema NUMERIC,
                ...
                last_modified_date TIMESTAMPTZ NOT NULL,
                CONSTRAINT in_weekly_pkey PRIMARY KEY (datetime, stock)
            """)

            # Create IN quarterly data table
            create_table(cur, 'in_quarterly_table', """
                datetime TIMESTAMPTZ NOT NULL,
                stock TEXT NOT NULL,
                sales NUMERIC,
                ebitda NUMERIC,
                ...
                ev_ebitda DECIMAL,
                last_modified_date TIMESTAMPTZ NOT NULL,
                CONSTRAINT in_quarterly_pkey PRIMARY KEY (datetime, stock)
            """)

            # Create indexes for all tables
            for table, columns in {
                'in_screener_table': ['stock', 'market_cap', 'pe_ratio', 'pb_ratio', 'peg_ratio', 
                             'sales_change_percent', 'ebitda_change_percent'],
                'in_alerts_table': ['stock'],
                'in_daily_table': ['stock'],
                'in_weekly_table': ['stock'],
                'in_quarterly_table': ['stock'],
                'in_insider_trading_table': ['stock', 'insider_name', 'transaction_type', 'total_value', 'officer_title']
            }.items():
