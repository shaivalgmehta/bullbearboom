    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_URL}/in_stocks/${symbol}/heikin-ashi?timeframe=${timeframe}`
        );
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <Paper sx={{ p: 2 }}>
          ...
          <Typography variant="body2">Open: {item.ha_open.toFixed(2)}</Typography>
          <Typography variant="body2">High: {item.ha_high.toFixed(2)}</Typography>
          <Typography variant="body2">Low: {item.ha_low.toFixed(2)}</Typography>
          <Typography variant="body2">Close: {item.ha_close.toFixed(2)}</Typography>
